...
var contentReport1 = {
  background: {
    ...
    {
      "HFAZ0193": {
        "description": "HFAZ0193: sda", "severity": "high", "priority": "high1",
        "accTable": [["Year", "2010", "2011"], ["No. of Accidents", "7", "77"],
        ["No. of Fatalities", "8", "9"]]
      },
      "HFAZ0008": {
        "description": "HFAZ0008: sda", "severity": "sev1", "priority": "p1",
...
function MergeAll() {
  ...
  const [auditID1, setAuditID] = useState('')
  ...
  const submitPDF = (x, y) => {
    ...
    AxiosApp.post(url1 + "merge_report", f1)
      .then((response) => {
  ...
  const generatePDF = useReactToPrint({
    ...
    print: async (printIframe) => {
      ...
      if (document) {
        ...
        const options = {
          margin: 1.5,
          filename: sId + "_merged_report" + ".pdf",
  ...
  const loadLists = () => {
    ...
    let l1 = { "audit_type_id": sId }
    AxiosApp.post(url1 + "report_merger", l1)
      .then((response) => {
        if (response.data.statusCode == "200") {
          let mr1 = {};
          //mr1.acc_data_table = {};
          ...
          mr1.exec_summary = {};
          mr1.acc_data_table = {}
          mr1.bgAccidentDetails = {};

          let s2 = response.data.details;
          let temp1 = "";
          let temp2 = "";
          let temp3 = "";
          let temp4 = "";
          let temp5 = "";
          for (let index = 0; typeof (s2[index]) == "object"; index++) {
            temp1 = (temp1 != "") ? temp1 + " , " + s2[index].acc_summary : s2[index].acc_summary;
            temp2 = (temp2 != "") ? temp2 + " , " + s2[index].bkg_summary : s2[index].bkg_summary;
            temp3 = (temp3 != "") ? temp3 + " , " + s2[index].proj_details : s2[index].proj_details;
            temp4 = (temp4 != "") ? temp4 + " , " + s2[index].opportunities : s2[index].opportunities;
            temp5 = (temp5 != "") ? temp5 + " , " + s2[index].exec_summary : s2[index].exec_summary;
          }
          mr1.acc_summary = temp1
          mr1.bkg_summary = temp2;
          mr1.proj_details = temp3;
          mr1.opportunities = temp4;
          mr1.exec_summary = temp5;
          ...
          let f = 0;
          for (let index = 0; typeof (s2[index]) == "object"; index++) {
            const element = s2[index].acc_data_table;
            a = (a != "") ? a + " , " + element.bs : element.bs;
            b = (b != "") ? b + " , " + element.bs_irc : element.bs_irc;
            c = (c != "") ? c + " , " + element.ebs : element.ebs;
            d = (d != "") ? d + " , " + element.pass_count_report : element.pass_count_report;
            e = (e != "") ? e + " , " + element.ped_count_report : element.ped_count_report;
            f = (f != "") ? f + " , " + element.veh_count_report : element.veh_count_report;
          }
          mr1.acc_data_table = {
            "bs": a,
            "bs_irc": b,
          ...
          for (let index = 0; typeof (s2[index]) == "object"; index++) {
            ...
            delete element.bgAccidentDetails.col_names;
            let onlyValues = Object.values(element.bgAccidentDetails)
            temp6 = [...temp6, ...onlyValues];
          }
          mr1.bgAccidentDetails = temp6
          ...
          try {
            for (let index = 0; typeof (s2[index]) == "object"; index++) {
              ...
              for (let index = 0; index < s1.length; index++) {
                const element = s1[index].hfazDetails
                for (const key in element) {
                  temp7[key] = element[key]
                }
              }
            }
          } catch (error) {
          }
          mr1.hfazDetails = temp7
          contentReport.background = mr1;
  ...
  async function loadAuditList() {
    let l1 = { "audit_type_id": sId }
    await AxiosApp.post(url1 + "stretchwise_audit", l1)
      .then((response) => {
        if (response.data.statusCode == "200") {
          setAuditID(response.data.details[0])
          setAuditList(response.data.details)
          auditID = response.data.details[0]

          for (let index = 0; index < response.data.details.length; index++) {
            loadAnsImages(response.data.details[index]);
          }
          populateTeamDetails();
          populateSalientReport(); //auditid
          loadHfazTableReport();
          getAllSubSection();//auditid
          loadCriticalSection();
          loadGeneralSection();
        }
      })
  ...
  const loadHfazTableReport = () => {
    ...
    AxiosApp.post(url1 + "hfaz_report_merged", l1)
      .then((response) => {
        if (response.data.statusCode == "200") {
          console.log(response.data);
          ...
          for (let index = 0; index < l1.length; index++) {
            ...
            loadIssuesPerHfaz(element.audit_id)
          }
        }
      })
      .catch((error) => {
  ...
  const loadCriticalSection = () => {
    ...
    AxiosApp.post(url1 + "critical_observation_merged", l1)
      .then((response) => {
        if (response.data.statusCode == "200") {
          console.log(response.data);
          let l1 = response.data.details;
  ...
  const loadGeneralSection = () => {
    ...
    AxiosApp.post(url1 + "general_observation_merged", l1)
      .then((response) => {
        if (response.data.statusCode == "200") {
          console.log(response.data);
          let l1 = response.data.details;
  ...
  return (<>
    ...
    {dummy && <div style={{ display: 'none' }}>{dummy}</div>}
    {Object.keys(contentReport.background).length == 0 && (
      <div>
        Loading....
      </div>
    )}
                  ...
                  >
                      ...
                      >
                        ...
                        <tr className="table_row_rsa">
                          ...
                          <td colspan="2" className="table_row_td">
                            {Object.values(contentReport.background.hfazDetails)[index] &&
                    ...
                    <div style={{ padding: "20px" }}>
                      ...
                      >
                        {/** hfaz acc data table */}
                        {Object.values(contentReport.background.hfazDetails)[index] &&
                          Object.values(contentReport.background.hfazDetails)[index].accTable &&
