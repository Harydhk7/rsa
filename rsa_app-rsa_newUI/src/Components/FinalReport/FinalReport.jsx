...
function FinalReport(props) {
      ...
      if (document) {
        ...
        const options = {
          ...
          html2canvas: {
            ...
            useCORS: true,    // Enable CORS if you're using external resources
            letterRendering: true,  // Render fonts properly
            logging: true,    // Enable logs to troubleshoot issues
            allowTaint: true,  // Allow tainted content
            foreignObjectRendering: true,
          },
          jdPDF: { unit: "mm", format: "a4" },
          jsPDF: { format: 'a4' },
          autoPaging: true,  // Enable auto paging (page breaks), 
          compress: true,
        };
  ...
  const loadAnsImages = () => {
    ...
    // return;
    setIsload(true);
    AxiosApp.post(url1 + "ans_images", JSON.stringify(config),
      {
          ...
          .then(zip => {
            ...
            imageFiles.forEach((filename, index) => {
              ...
              zip.file(filename)
                .async('blob')
                .then(blob => {
                  imageSource1.set(filename, URL.createObjectURL(blob))
                  ...
                  imageFileSource1.set(filename, zfile1)
                });
            })
            console.log(imageSource1)
            console.log(imageFileSource1);
  ...
  const loadEditableDatas = () => {
    ...
    AxiosApp.post(url1 + "report_rsa_data", {
      "audit_id": auditID
    })
      .then((response) => {
  ...
  const fetchImageURLSalient = (mode) => {
    ...
    if (!mode) return returnArray;
      ...
      search1 = (mode == "start") ? ".A.3.webp" : ".B.3.webp";
  ...
  const fetchImageURL = (l1, rowNo) => {
    ...
    let search1 = auditID + "/" + rowNo + "/" + l1 + ".webp";
  ...
  const loadGeneralSection = () => {
    ...
    axios.post(url1 + "general_observation", {
      "audit_id": auditID
    })
      .then((response) => {
        if (response.data.statusCode == "200") {
    ...
    {Object.keys(contentReport).length > 0 && (
      <div id="rsa_report_id" style={{ padding: "20px" }}>
        ...
        <div ref={pdfRef} className="pdf-container">
          ...
          {/* page three */}
          <div className="brkAlways" style={{ marginTop: "20px" }}>
            <p className="font12Print"
              style={{
                // lineHeight: "28px",
              }}
          ...
          <div className="brkAlways" style={{ marginTop: "20px" }}>
            ...
            <div className="brkBefore brkAvoid brkAlways">
              <p className="font12Print"
                style={{
                  // lineHeight: "28px",
                  marginTop: "30px",
            ...
            {generalSection.length > 0 && <>
              ...
              <div className="stretch_details_rsa hfaztabone landscape_section"
                ...
                {generalSection.map((i2, j1) => {
                  return (
                    <div className="landscape_section brkAvoid" style={{ marginTop: "20px", marginBottom: "20px" }}>
                      <br />
