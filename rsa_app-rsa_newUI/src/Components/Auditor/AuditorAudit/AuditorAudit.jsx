...
function AuditorAudit() {
  ...
  const auditorID = localStorage.getItem("rsa_user1")

  const loadLists = () => {
    let l1 = { "user_id": auditorID }

    AxiosApp.post(url1 + "auditor_dashboard", l1)
      .then((response) => {
        setIsload(false);
        let l2 = response.data.details;
        //add filter conditions if BE is not doing
        /**
         * 1. fielduser is not null and not empty and 
         * status is audit completed
         */
        let l1 = l2;
