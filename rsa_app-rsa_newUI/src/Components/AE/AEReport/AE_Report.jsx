...
import AE_DataAnalysis from "../AEAudit/AE_DataAnalysis";
...
function AE_Report() {
  ...
  const [isload, setIsload] = useState("");
    ...
    const [uatyid, setAdTyId] = useState("");
      ...
      const [openEditReport, setopenEditReport] = React.useState(false);
  ...
  const handleOpenEditReport = (aid, atypeID) => {

    console.log("aid >> " + aid," type id >> " + atypeID)
    setuAdiId(aid);
    setAdTyId(atypeID);
    handleOpenEditReportModal();
    // navigate("/Report_final", { aid: params.row.audit_id})
    
};



  const handleOpenEditReportModal = () => {
    setopenEditReport(true);

    console.log("first");
  };
  const handleCloseEditReportModal = () => setopenEditReport(false);
        ...
        return (
          ...
          >
            <Button
              ...
              onClick={() => {
                navigate("/Ae_dataAnalysis", {
                  state: {
                    aid: params.row.audit_id
                    , uid: localStorage.getItem("rsa_user1"), 
                    atypeID:params.row.audit_type, editMode: true
                  }
               })}
              // onClick={() => {
              //   handleOpenEditReport(params.row.audit_id, params.row.audit_type, true)
              // }
              }
            >
              <DescriptionIcon />
        ...
        return (
          <Button
            style={{
             ...
             onClick={() => navigate("/AE_FinalReport", { state: { aid: l3, atid: l4 } })}
          >
            View and Submit
          </Button> 
        );
  ...
  function createData(id, audit_type,  stretch_name,road_type,stage, audit_id, audit_type_id, status) {
    return { id, audit_type,  stretch_name,road_type,stage, audit_id, audit_type_id,status };
  }
  const loadLists = () => {
    let l1 = { "userid": localStorage.getItem("rsaLogged") }
      ...
      .then((response) => {
        ...
        if (response.data.statusCode == "200") {
          ...
          for (let element of response.data.details) {
            r1.push(createData(++count, element['audit_plan_type'], 
              element['stretch_name'], element['road_type'], element['stage'], 
              ...
              {openEditReport && (
                ...
                >
                  <Box sx={style}>
                    ...
                    {/* <FinalReport /> */}
                    <AE_DataAnalysis aid={uadid} atid={uatyid} />
                    <div
                      style={{
                        display: "flex",
                        justifyContent: "center",
